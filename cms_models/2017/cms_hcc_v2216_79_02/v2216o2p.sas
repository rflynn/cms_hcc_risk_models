 ...
 *   DESCRIPTION:
 *
 * V2216O2P program creates seventy nine HCC variables version 22
 ...
 *                Medicare data)
 *     :DIAG    - diagnosis
 *     :DIAG_TYPE - diagnosis version ('9' for ICD9 and '0' for ICD10)
 *
 * The program supplies parameters to a main macro %V2216O2M that calls
 * other external macros specific to V22 HCCs:
 *
 *      %AGESEXV2   - create age/sex, originally disabled, disabled vars
 *      %V22I9ED1   - perform edits to ICD9 diagnosis
 *      %V22I0ED1   - perform edits to ICD10 diagnosis
 ...
 *                   - perform ICD9 edits using V22I9ED1 macro or
 *                     perform ICD10 edits using V22I0ED1 macro
 ...
 ***********************************************************************;
 ...
 /*
 the definitions below need to be tailored to the particular directory
 structure on your machine
*/

 %LET MODELDIR=/folders/myfolders/sasuser.v94/cms_hcc_2017;
 LIBNAME LIBRARY "&MODELDIR";
 FILENAME IN0 "&MODELDIR";
 LIBNAME  IN1 "&MODELDIR/input";
 LIBNAME  IN2 "&MODELDIR/input";
 LIBNAME  INCOEF "&MODELDIR";
 LIBNAME  OUT "&MODELDIR/output";

 proc import datafile="&MODELDIR/input/person.csv"
 dbms=csv
 out=IN1.PERSON;
 run;

 proc import datafile="&MODELDIR/input/diag.csv"
 dbms=csv
 out=IN2.DIAG;
 run;

 filename inc "&MODELDIR/C2214O5P";
